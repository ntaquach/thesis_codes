...
```{r, warning=F,message=F}
library(readxl)
density_combined <- read_excel("~/thesis-codes/density_combined.xlsx", 
     sheet = "Final")
```
...
m_density <- as.matrix(density_wo_su)
```
...
set.seed(1)
nmds_density <- metaMDS(m_density,distance="bray")
```
...
Another way to add columns to the existing dataset is to use pipes from the dplyr package (2).
...
Package ggplot2 is always the way to go when plotting (3).
