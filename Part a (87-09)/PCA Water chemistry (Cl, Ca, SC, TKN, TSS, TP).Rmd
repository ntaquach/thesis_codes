...
These are the codes I used to run PCA of my water chemistry data. Instead of using the princomp() function, I am going to try out the rda() function from the vegan package (Oksanen et al. 2012). Any comment is welcome. 
...
```{r message=F,warning=F}
wc_wo_SU <- wc[,4:ncol(wc)] #exclude the first column that contains SU information
...
wc_pca_plot <- ggplot(site.scores.dplyr,aes(PC1, PC2))+ 
    geom_point(size = 4,aes(color=Subwatershed, shape = as.character(Year))) +
